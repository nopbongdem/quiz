            ...
            <div class="box">
                <div class="box-header">
                    ...
                    @if(Entrust::can('create-permission'))
                        <a href="{!! url('/admin/manager/permission/create') !!}" class="btn btn-info pull-right">{!! BS3::icon('plus') !!}</a>
                    @endif
                </div><!-- /.box-header -->
                <div class="box-body">
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="...">
                                    @if(Entrust::can('edit-permission'))
                                        <a href="{!! url('/admin/manager/permission/'.$data->id.'/edit') !!}" class="btn btn-primary">{!! FA::icon('pencil') !!}</a>
                                    @endif

                                    @if(Entrust::can('delete-permission'))
                                        <a class="btn btn-danger" href="{{ route('admin.manager.permission.destroy',array($data->id)) }}" data-method="delete" rel="nofollow" data-confirm="{!! trans('labels.delete_confirm') !!}">{!! FA::icon('trash') !!}</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
